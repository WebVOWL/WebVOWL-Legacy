...
export default class NodeDegreeFilter extends AbstractFilter {
    ...
    initialize(nodes, properties) {
        ...
        this.menu.defaultDegreeValue = this.#findAutoDefaultDegree(
            nodes,
            properties,
            maxLinkCount,
            linkCounts,
    ...
    #findAutoDefaultDegree(nodes, properties, maxDegree, linkCounts) {
        ...
        try {
            if (linkCounts.length < this.NODE_COUNT_LIMIT_FOR_AUTO_ENABLING) {
                const filteredData = this.#filterByNodeDegree(
                    nodes,
                    properties,
                    0,
                )
                if (
                    ...
                    this.NODE_COUNT_LIMIT_FOR_AUTO_ENABLING
                ) {
                    return 0
                }
            } else {
                const degree =
                    linkCounts[this.NODE_COUNT_LIMIT_FOR_AUTO_ENABLING - 1]
                const filteredData = this.#filterByNodeDegree(
                    nodes,
                    properties,
                    degree,
        ...
        console.log(
            "Cannot determine degree of collapse. Value is set to max to avoid a crash.",
        )
        return maxDegree
    }
    ...
    #filterByNodeDegree(nodes, properties, minDegree) {
        return FilterTools.filterNodesAndTidy(
            nodes,
            properties,
            this.#hasRequiredDegree(minDegree),
        )
    }
