...
let filterTools = require("../util/filterTools")();
...
module.exports = function (menu) {
  ...
  let NODE_COUNT_LIMIT_FOR_AUTO_ENABLING = 50;


  filter.initialize = function (nodes, properties) {
    lastFiltedDegree = -1;
    let linkCounts = findLinkCounts(nodes);
    let maxLinkCount = linkCounts[0];
    ...
    let defaultDegree = findDefaultDegree(maxLinkCount);
  ...
  function findAutoDefaultDegree(nodes, properties, maxDegree, linkCounts) {
    ...
    try {
      if (linkCounts.length < NODE_COUNT_LIMIT_FOR_AUTO_ENABLING) {
        let filteredData = filterByNodeDegree(nodes, properties, 0);
        if (filteredData.nodes.length <= NODE_COUNT_LIMIT_FOR_AUTO_ENABLING) {
          return 0;
        }
      } else {
        let degree = linkCounts[NODE_COUNT_LIMIT_FOR_AUTO_ENABLING - 1];
        let filteredData = filterByNodeDegree(nodes, properties, degree);
        if (filteredData.nodes.length <= NODE_COUNT_LIMIT_FOR_AUTO_ENABLING) {
          return degree;
        } else {
          return degree + 1;
  ...
  function findDefaultDegree(maxDegree) {
    let globalDegOfFilter = menu.getGraphObject().getGlobalDOF();
  ...
  function findLinkCounts(nodes) {
    ...
    for (let i = 0, l = nodes.length; i < l; i++) {
      let linksWithoutDatatypes = filterOutDatatypes(nodes[i].links());
  ...
  function filterByNodeDegreeAndApply(minDegree) {
    let filteredData = filterByNodeDegree(nodes, properties, minDegree);
