...
module.exports = function (graph) {
    ...
    editSidebar.updatePrefixUi = function () {
        ...
        let prefixListContainer = d3.select("#prefixURL_Container");
    ...
    function setupSupportedDatatypes() {
        let datatypeEditorSelection = d3.select("#typeEditor_datatype").node();
        let supportedDatatypes = ["undefined", "xsd:boolean", "xsd:double", "xsd:integer", "xsd:string"];
    ...
    function highlightDeleteButton(enable, name) {
        let deletePath = d3.select("#del_pathFor_" + name);
        let deleteRect = d3.select("#del_rectFor_" + name);
    ...
    function highlightEditButton(enable, name, fill) {
        let editPath = d3.select("#pathFor_" + name);
        let editRect = d3.select("#rectFor_" + name);
    ...
    function setupAddPrefixButton() {
        ...
        btn.on("click", function (event) {
            ...
            if (prefix_editMode === false) {
                ...
                let editIcon = editButton.append("g");
                let editRect = editIcon.append("rect");
                let editPath = editIcon.append("path");
                ...
                editButton.selectAll("g").on("mouseover", function (event) {
                    highlightEditButton(true, this.selectorName, true);
                });
                editButton.selectAll("g").on("mouseout", function (event) {
                ...
                let prefInput = prefixEditContainer.append("input");
                ...
                let prefURL = prefixEditContainer.append("input");
                ...
                let deleteContainer = prefixEditContainer.append("div");
                deleteContainer.style("float", "right");
                let deleteButton = deleteContainer.append("svg");
                ...
                let deleteIcon = deleteButton.append("g");
                let deleteRect = deleteIcon.append("rect");
                let deletePath = deleteIcon.append("path");
                ...
                deleteButton.selectAll("g").on("mouseover", function (event) {
                    highlightDeleteButton(true, this.selectorName);
                });
                deleteButton.selectAll("g").on("mouseout", function (event) {
    ...
    function setupPrefixList() {
        ...
        let prefixListContainer = d3.select("#prefixURL_Container");
        let prefixElements = graph.options().prefixList();
        for (let name in prefixElements) {
            if (prefixElements.hasOwnProperty(name)) {
                ...
                editButton.selectAll("g").on("mouseover", function (event) {
                    ...
                    let fill = false;
                    let enable = true;
                    let f_editPath = d3.select("#pathFor_" + sender.selectorName);
                    let f_editRect = d3.select("#rectFor_" + sender.selectorName);
                ...
                deleteButton.selectAll("g").on("mouseover", function (event) {
                    let selector = this;
                    let enable = true;
                    let f_deletePath = d3.select("#del_pathFor_" + selector.selectorName);
                    let f_deleteRect = d3.select("#del_rectFor_" + selector.selectorName);
    ...
    function deletePrefixLine(event) {
        if (this.disabled === true) {
            return;
        }
        d3.select("#addPrefixButton").node().innerHTML = "Add Prefix";
        let selector = this.id.split("_")[1];
    ...
    function enablePrefixEdit(event, item) {
        let agent = this;
        if (item)
            agent = item;

        if (agent.disabled === true) return;
        let selector = agent.id.split("_")[1];
        let stl = agent.elementStyle;
        if (stl === "edit") {
            ...
            let editButton = d3.select(agent);
            editButton.selectAll("g").on("mouseover", function (event) {
            ...
            let editPath = d3.select("#pathFor_" + agent.selectorName);
            editPath.attr("d", "M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z");
            editPath.attr("transform", "matrix(0.45,0,0,0.45,0,5)");
        ...
        if (stl === "save") {
            let newPrefixURL = d3.select("#prefixURLFor_" + selector).node().value;
            let newPrefix = d3.select("#prefixInputFor_" + selector).node().value;
            ...
            if (graph.options().updatePrefix(oldPrefix, newPrefix, oldPrefixURL, newPrefixURL) === true) {
                ...
                prefixModule.updatePrefixModel();
                let saveButton = d3.select(agent);
                saveButton.selectAll("g").on("mouseover", function (event) {
                    highlightEditButton(true, agent.selectorName, false);
                });
                saveButton.selectAll("g").on("mouseout", function (event) {
                    highlightEditButton(false, agent.selectorName, false);
                });

                let savePath = d3.select("#pathFor_" + agent.selectorName);
    ...
    function changeDatatypeType(element) {
        let datatypeEditorSelection = d3.select("#typeEditor_datatype").node();
        let givenName = datatypeEditorSelection.value;
    ...
    function getURLFROMPrefixedVersion(element) {
        ...
        let base = graph.options().getGeneralMetaObjectProperty("iri");
        if (validURL(url) === false) {
            ...
            if (tokens.length === 2) {
                ...
                let name = tokens[1];
                if (pr.length > 0) {
                    let basePref = graph.options().prefixList()[pr];
    ...
    function changeIriForElement(event, element) {
        let url = getURLFROMPrefixedVersion(element);
        let base = graph.options().getGeneralMetaObjectProperty("iri");
    ...
    editSidebar.updateSelectionInformation = function (element) {
        ...
        if (element === undefined) {
            ...
            d3.select("#element_iriEditor")
                .on("change", function (event) {
                    let elementIRI = element.iri();
                    let prefixed = graph.options().prefixModule().getPrefixRepresentationForFullURI(elementIRI);
                    if (prefixed === d3.select("#element_iriEditor").node().value) {
                        console.log("Iri is identical, nothing has changed!");
                        return;
                    }

                    changeIriForElement(event, element);
                })
                .on("keydown", function (event) {
                    event.stopPropagation();
                    if (event.code === 13) {
            ...
            let forceIRISync = defaultIriValue(element);
            d3.select("#element_labelEditor")
                ...
                .on("keyup", function (event) {
                    if (forceIRISync) {
                        let labelName = d3.select("#element_labelEditor").node().value;
                        let resourceName = labelName.replaceAll(" ", "_");
                        let syncedIRI = element.baseIri() + resourceName;
            ...
            if (element.type() === "rdfs:Datatype") {
                let datatypeEditorSelection = d3.select("#typeEditor_datatype");
                ...
                datatypeEditorSelection.on("change", function (event) {
                    changeDatatypeType(element);
                });
            }

            // add type selector
            let typeEditorSelection = d3.select("#typeEditor").node();
            let htmlCollection = typeEditorSelection.children;
            let numEntries = htmlCollection.length;
            for (let i = 0; i < numEntries; i++)
                typeEditorSelection.removeChild(htmlCollection[0]);

            let elementPrototypes = getElementPrototypes(element);
            ...
            d3.select("#typeEditor").on("change", function (event) {
                elementTypeSelectionChanged(event, element);
            });

            // add characteristics selection
            let needChar = elementNeedsCharacteristics(element);
            d3.select("#property_characteristics_Container").classed("hidden", !needChar);
            if (needChar === true) {
                addElementsCharacteristics(element);
            }
            let fullURI = d3.select("#element_iriEditor").node().value;
    ...
    editSidebar.updateGeneralOntologyInfo = function () {
        let preferredLanguage = graph && graph.language ? graph.language() : null;
        // get it from graph.options
        let generalMetaObj = graph.options().getGeneralMetaObject();
    ...
    editSidebar.updateElementWidth = function () {
        ...
        let lsb_offset = d3.select("#logo").node().getBoundingClientRect().height + 5;
        let lsb_height = height - lsb_offset;
        ...
        let title_labelWidth = d3.select("#titleEditor-label").node().getBoundingClientRect().width + 20;
        let iri_labelWidth = d3.select("#iriEditor-label").node().getBoundingClientRect().width + 20;
        let version_labelWidth = d3.select("#versionEditor-label").node().getBoundingClientRect().width + 20;
        let author_labelWidth = d3.select("#authorsEditor-label").node().getBoundingClientRect().width + 20;
        //find max width;
        let maxW = 0;
        ...
        let meta_inputWidth = div_width - maxW - 10;
        ...
        let elementIri_width = d3.select("#element_iriEditor-label").node().getBoundingClientRect().width + 20;
        let elementLabel_width = d3.select("#element_labelEditor-label").node().getBoundingClientRect().width + 20;
        let elementType_width = d3.select("#typeEditor-label").node().getBoundingClientRect().width + 20;
        let elementDType_width = d3.select("#typeEditor_datatype-label").node().getBoundingClientRect().width + 20;
        ...
        let selectedElement_inputWidth = div_width - maxW - 10;
        ...
        let containerWidth = d3.select("#containerForPrefixURL").node().getBoundingClientRect().width;
        if (containerWidth !== 0) {
            let inputs = d3.selectAll(".prefixInput");
            if (inputs.node()) {
                let prefixWidth = d3.selectAll(".prefixInput").node().getBoundingClientRect().width;
    ...
    function addElementsCharacteristics(element) {
        ...
        let i;
        // KILL old elements
        let charSelectionNode = d3.select("#property_characteristics_Selection");
        let htmlCollection = charSelectionNode.node().children;
        if (htmlCollection) {
            let numEntries = htmlCollection.length;
        ...
        if (element.attributes().indexOf("external") > -1) {
            ...
            let externalCharSpan = charSelectionNode.append("span");
        ...
        if (elementTools.isNode(element) === true) {
            ...
            let arrayOfNodeChars = ["deprecated"];
    ...
    function handleCheckBoxClick(event) {
        ...
        let char = this.getAttribute("characteristics");
        if (checked === true) {
    ...
    function addAttribute(selectedElement, char) {
        ...
        if (selectedElement.indications().indexOf(char) === -1) {
            let indications = selectedElement.indications();
        ...
        if (getPresentAttribute(selectedElement, "external") && getPresentAttribute(selectedElement, "deprecated")) {
            visAttr = selectedElement.visualAttributes();
            let visInd = visAttr.indexOf("external");
            if (visInd > -1) {
    ...
    function removeAttribute(selectedElement, element) {
        ...
        let attrInd = attr.indexOf(element);
        if (attrInd >= 0) {
            attr.splice(attrInd, 1);
        }
        let indInd = indications.indexOf(element);
        if (indInd > -1) {
            indications.splice(indInd, 1);
        }
        let visInd = visAttr.indexOf(element);
        if (visInd > -1) {
            visAttr.splice(visInd, 1);
        }
        selectedElement.attributes(attr);
    ...
    function elementTypeSelectionChanged(event, element) {
        if (elementTools.isNode(element)) {
            if (graph.changeNodeType(event, element) === false) {
                //restore old value
                editSidebar.updateSelectionInformation(element);
            }
        }
        ...
        if (elementTools.isProperty(element)) {
            if (graph.changePropertyType(event, element) === false) {
                //restore old value
                editSidebar.updateSelectionInformation(element);
            }
        }
    ...
    function setupCollapsing() {
        ...
        let triggers = d3.selectAll(".accordion-trigger");
        ...
        triggers.on("click", function (event) {
            let selectedTrigger = d3.select(this);
