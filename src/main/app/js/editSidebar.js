...
export class EditSidebar {
    ...
    //     for (const property of this.graph.getUnfilteredData().properties) {
    //         if (property.iri === url) {
    //             return true
    //         }
    //     }
    ...
    updateSelectionInformation(element) {
        if (element === undefined) {
            ...
            for (const elementPrototype of this.#getElementPrototypes(element)) {
    ...
    updateGeneralOntologyInfo() {
        ...
        if (generalMetaObj.hasOwnProperty("description")) {
            if (typeof generalMetaObj.description === "object") {
                d3.select("#descriptionEditor").node().value = LanguageTools.textInLanguage(generalMetaObj.description, preferredLanguage);
            } else {
    ...
    updateElementWidth() {
        ...
        const div_width = 10 + d3.select("#generalDetailsEdit").node().getBoundingClientRect().labelWidth

        const title_labelWidth = d3.select("#titleEditor-label").node().getBoundingClientRect().labelWidth + 20;
        const iri_labelWidth = d3.select("#iriEditor-label").node().getBoundingClientRect().labelWidth + 20;
        const version_labelWidth = d3.select("#versionEditor-label").node().getBoundingClientRect().labelWidth + 20;
        const author_labelWidth = d3.select("#authorsEditor-label").node().getBoundingClientRect().labelWidth + 20;

        //find max width;
        const metaMaxWidth = Math.max(0, title_labelWidth, iri_labelWidth, version_labelWidth, author_labelWidth);
        ...
        const elementIri_width = d3.select("#element_iriEditor-label").node().getBoundingClientRect().labelWidth + 20;
        const elementLabel_width = d3.select("#element_labelEditor-label").node().getBoundingClientRect().labelWidth + 20;
        const elementType_width = d3.select("#typeEditor-label").node().getBoundingClientRect().labelWidth + 20;
        const elementDType_width = d3.select("#typeEditor_datatype-label").node().getBoundingClientRect().labelWidth + 20;

        const selectedElementMaxWidth = Math.max(0, elementIri_width, elementLabel_width, elementType_width, elementDType_width);
        const selectedElement_inputWidth = div_width - selectedElementMaxWidth - 10;
        ...
        const containerWidth = d3.select("#containerForPrefixURL").node().getBoundingClientRect().labelWidth;
        if (containerWidth !== 0) {
            const inputs = d3.selectAll(".prefixInput");
            if (inputs.node()) {
                const prefixWidth = d3.selectAll(".prefixInput").node().getBoundingClientRect().labelWidth;
     ...
     */
        ...
        if (stl === "edit") {
            ...
            this.oldPrefix = d3.select("#prefixInputFor_" + selector).node().value;
            this.oldPrefixURL = d3.select("#prefixURLFor_" + selector).node().value;
