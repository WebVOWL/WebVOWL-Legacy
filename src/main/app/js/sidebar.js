...
module.exports = function (graph) {
    ...
    sidebar.clearOntologyInformation = function () {
        ...
        let container = d3.select("#ontology-metadata");
        ...
        let isTriggerActive = d3.select("#selection-details-trigger").classed("accordion-trigger-active");
        if (isTriggerActive) {
            // close accordion
    ...
    function setLanguages(languages) {
        ...
        let languageSelection = d3.select("#language")
            .on("change", function (event) {
                graph.language(event.target.value);
                updateGraphInformation();
                sidebar.updateSelectionInformation(event, lastSelectedElement);
            });
    ...
    function trySelectDefaultLanguage(selection, languages, language) {
        let langIndex = languages.indexOf(language);
        if (langIndex >= 0) {
    ...
    function updateGraphInformation() {
        let title = languageTools.textInLanguage(ontologyInfo.title, graph.language());
        ...
        let authors = ontologyInfo.author;
        ...
        let description = languageTools.textInLanguage(ontologyInfo.description, graph.language());
    ...
    function listAnnotations(container, annotationObject) {
        ...
        let annotations = [];
        for (let annotation in annotationObject) {
     ...
     */
    sidebar.updateSelectionInformation = function (event, selectedElement) {
    ...
    function displayPropertyInformation(property) {
        ...
        let equivalentIriSpan = d3.select("#propEquivUri");
    ...
    function displayAttributes(attributes, textSpan) {
        let spanParent = d3.select(textSpan.node().parentNode);
    ...
    function displayNodeInformation(node) {
        ...
        /* Equivalent stuff. */
        let equivalentIriSpan = d3.select("#classEquivUri");
        ...
        /* Disjoint stuff. */
        let disjointNodes = d3.select("#disjointNodes");
        let disjointNodesParent = d3.select(disjointNodes.node().parentNode);
    ...
    function setTextAndVisibility(label, value) {
        let parentNode = d3.select(label.node().parentNode);
        let hasValue = !!value;
        if (value) {
            label.text(value);
        }
    ...
    sidebar.updateSideBarVis = function (init) {
        let vis = sidebar.getSidebarVisibility();
        sidebar.showSidebar(parseInt(vis), init);
    };

    sidebar.getSidebarVisibility = function () {
        let isHidden = detailArea.classed("hidden");
    ...
    sidebar.setup = function () {
        ...
        collapseButton.on("click", function (event) {
            ...
            let settingValue = parseInt(sidebar.getSidebarVisibility());
