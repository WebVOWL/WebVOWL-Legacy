...
module.exports = function (graph) {
    ...
    gravityMenu.setup = function () {
        let menuEntry = d3.select("#m_gravity");
    ...
    function addDistanceSlider(selector, identifier, label, distanceFunction) {
        let defaultLinkDistance = distanceFunction();
        ...
        slider.on("focusout", function (event) {
            graph.updateStyle();
        });

        slider.on("input", function (event) {
            let distance = slider.property("value");
    ...
    function adjustCharge(defaultLinkDistance) {
        let greaterDistance = Math.max(options.classDistance(), options.datatypeDistance()),
