...
module.exports = function (graph) {
    ...
    modeMenu.setup = function (pickAndPin, nodeScaling, compactNotation, colorExternals) {
        ...
        let container = addModeItem(colorExternals, "colorexternals", "Color externals", "#colorExternalsOption", true);
    ...
    function addCheckBoxD(identifier, modeName, selector, onChangeFunc, updateLvl) {
        let moduleOptionContainer = d3.select(selector)
            .append("div")
            .classed("checkboxContainer", true);

        let moduleCheckbox = moduleOptionContainer.append("input")
            .classed("moduleCheckbox", true)
            .attr("id", identifier + "ModuleCheckbox")
            .attr("type", "checkbox")
            .property("checked", onChangeFunc());

        moduleCheckbox.on("click", function (event, d) {
            let isEnabled = moduleCheckbox.property("checked");
    ...
    function addModeItem(module, identifier, modeName, selector, updateGraphOnClick) {
        ...
        moduleCheckbox.on("click", function (event, d, silent) {
            let isEnabled = moduleCheckbox.property("checked");
    ...
    function addExternalModeSelection(container, colorExternalsMode) {
        let button = container.append("button").datum({ active: false }).classed("color-mode-switch", true);
        applyColorModeSwitchState(button, colorExternalsMode);

        button.on("click", function (event, silent) {
            let data = button.datum();
    ...
    function applyColorModeSwitchState(element, colorExternalsMode) {
        let isActive = element.datum().active;
        let activeColorMode = getColorModeByState(isActive);
