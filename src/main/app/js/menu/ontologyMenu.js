...
export default class OntologyMenu {
    ...
    getLoadingStatusOnceCallBacked(callback, parameter) {
        d3.xhr(WebVOWL.url_prefix + "loadingStatus?sessionId=" + this.conversion_sessionId, "application/text", (error, request) => {
            if (error) {
    ...
    callbackUpdateLoadingMessage(msg) {
        d3.xhr(WebVOWL.url_prefix + "loadingStatus", "application/text", (error, request) => {
            if (request !== undefined) {
                this.setLoadingStatusInfo(request.responseText + "<br>" + msg);
            } else {
    ...
    callbackLoad_Ontology_FromIRI(parameter) {
        ...
        this.stopTimer = false;
        this.timedLoadingStatusLogger();
        d3.xhr(WebVOWL.url_prefix + relativePath, "application/json", (error, request) => {
            ...
            if ((error !== null && error.status === 500) || (request && request.responseText.length === 0)) {
                clearTimeout(this.loadingStatusTimer);
                this.stopTimer = true;
                this.getLoadingStatusOnceCallBacked(this.callbackFromIRI_URL_ERROR, [error, request, localThreadId]);
            }
            if (loadingSuccessful) {
                clearTimeout(this.loadingStatusTimer);
                this.stopTimer = true;
                this.getLoadingStatusOnceCallBacked(this.callbackFromIRI_Success, [ontoName, localThreadId]);
            }
        });
    }
    ...
    callbackLoad_Ontology_From_DirectInput(text, parameter) {
        ...
        xhr.onload = () => {
            clearTimeout(this.loadingStatusTimer);
            this.stopTimer = true;
            this.getLoadingStatusOnceCallBacked(this.callbackForConvert, [xhr, input, sessionId]);
        };
        this.timedLoadingStatusLogger();
        xhr.send(formData);
    }
    ...
    callbackLoad_JSON_FromURL(parameter) {
        ...
        d3.xhr(WebVOWL.url_prefix + relativePath, "application/json", (error, request) => {
            ...
            if ((error !== null && error.status === 500) || (request && request.responseText.length === 0)) {
                ...
                this.getLoadingStatusOnceCallBacked(this.callbackFromJSON_URL_ERROR, [error, request, local_conversionId]);
            }
            if (loadingSuccessful) {
                clearTimeout(this.loadingStatusTimer);
                this.stopTimer = true;
                this.getLoadingStatusOnceCallBacked(this.callbackFromJSON_Success, [ontoName, local_conversionId]);
            }
        });
    };
    ...
    callbackLoadFromOntology(selectedFile, filename, local_threadId) {
        ...
        xhr.onload = () => {
            clearTimeout(this.loadingStatusTimer);
            this.stopTimer = true;
            this.getLoadingStatusOnceCallBacked(this.callbackForConvert, [xhr, filename, local_threadId]);
        };
        this.timedLoadingStatusLogger();
        xhr.send(formData);
    }
    ...
    conversionFinished(id) {
        ...
        d3.xhr(WebVOWL.url_prefix + "conversionDone?sessionId=" + local_id, "application/text", function (error, request) {
            if (error) {
