...
module.exports = function (graph) {
    ...
    filterMenu.setup = function (datatypeFilter, objectPropertyFilter, subclassFilter, disjointFilter, setOperatorFilter, nodeDegreeFilter) {
        ...
        menuControl.on("mouseover", function (event) {
            let searchMenu = graph.options().searchMenu();
            searchMenu.hideSearchEntries();
        });
        menuControl.on("mouseleave", function (event) {
    ...
    function addNodeDegreeFilter(nodeDegreeFilter, container) {
        ...
        degreeSlider.on("change", function (event, silent) {
            if (silent !== true) {
                graph.update();
        ...
        degreeSlider.on("input", function (event) {
            let degree = degreeSlider.property("value");
        ...
        degreeSlider.on("focusout", function (event) {
    ...
    function handleWheelEvent(event) {
        ...
        let maxDeg = parseInt(degreeSlider.attr("max"));
        let oldVal = parseInt(degreeSlider.property("value"));
    ...
    filterMenu.setCheckBoxValue = function (id, checked) {
        for (let i = 0; i < checkboxData.length; i++) {
            let cbdId = checkboxData[i].checkbox.attr("id");
            if (cbdId === id) {
