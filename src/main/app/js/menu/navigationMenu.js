...
module.exports = function (graph) {
    ...
    function setupControlsAndMenus() {
        ...
        let menuElements = menuContainer.children;
        ...
        d3.select("#scrollRightButton").on("mousedown", function (event) {
        ...
        d3.select("#scrollLeftButton").on("mousedown", function (event) {
            scrolLeftValue = scrollContainer.scrollLeft;
            navigationMenu.hideAllMenus();
        ...
        d3.select("#menuElementContainer").on("scroll", function (event) {
            navigationMenu.updateScrollButtonVisibility();
            navigationMenu.hideAllMenus();
        });
    }

    function menuElementOnHovered(event) {
        navigationMenu.hideAllMenus();
        if (touchedElement) {
            return;
        }
    ...
    function menuElementOutHovered(event) {
        hoveroutedControMenu(this.id);
    }

    function menuElementClicked(event) {
        let m_element = m_select[c_select.indexOf(this.id)];
        if (m_element) {
            let menuElement = d3.select("#" + m_element);
            if (menuElement) {
    ...
    function menuElementTouched(event) {
    ...
    function showSingleMenu(controllerID) {
        ...
        let m_element = m_select[c_select.indexOf(controllerID)];
        if (m_element) {
            if (controllerID !== "c_search") {
    ...
    function updateMenuPosition() {
        if (currentlyHoveredEntry) {
            let leftOffset = currentlyHoveredEntry.offsetLeft;
            let scrollOffset = scrollContainer.scrollLeft;
            let totalOffset = leftOffset - scrollOffset;
            let finalOffset = Math.max(0, totalOffset);
            let fullContainer_width = scrollContainer.getBoundingClientRect().width;
            let elementWidth = currentlyVisibleMenu.node().getBoundingClientRect().width;
            ...
            // let menuWidth=currentlyHoveredEntry.getBoundingClientRect().width;
            // let bt_width=36;
    ...
    navigationMenu.setup = function () {
        ...
        d3.select("#graph").on("mouseover", function (event) {
            navigationMenu.hideAllMenus();
        });
        d3.select("#generalDetails").on("mouseover", function (event) {
            navigationMenu.hideAllMenus();
        });
    };
