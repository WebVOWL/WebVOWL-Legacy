...
module.exports = function (graph) {
    ...
    let c_locate = d3.select("#locateSearchResult");
    let c_search = d3.select("#c_search");
    let m_search = d3.select("#m_search"); // << dropdown container;
    ...
    searchMenu.setup = function () {
        ...
        c_locate.on("click", function (event) {
            graph.locateSearchResult();
        });

        c_locate.on("mouseover", function (event) {
    ...
    function hoverSearchEntryView(event) {
    ...
    function toggleSearchEntryView(event) {
    ...
    function userNavigation(event) {
        ...
        if (event.code === 13) {
            if (selectedEntry >= 0 && selectedEntry < numEntries) {
                ...
                let iri = clearedText.replace(/ /g, "%20");

                let valid = ValidURL(iri);
                // validate url:
                if (valid) {
                    let ontM = graph.options().ontologyMenu();
                    ontM.setIriText(iri);
