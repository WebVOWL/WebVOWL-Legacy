...
module.exports = function (graph) {
    let zoomSlider = {};
    let minMag = graph.options().minMagnification(),
    ...
    zoomSlider.setup = function () {
        ...
        d3.select("#zoomOutButton").on("mousedown", function (event) {
        ...
        d3.select("#zoomInButton").on("mousedown", function (event) {
            graph.options().navigationMenu().hideAllMenus();
            zoomValue = graph.scaleFactor();
            t_zoomIn = requestAnimationFrame(timed_zoomIn);
        })
        ...
        d3.select("#centerGraphButton").on("click", function (event) {
            graph.options().navigationMenu().hideAllMenus();
    ...
    zoomSlider.zooming = function () {
        graph.options().navigationMenu().hideAllMenus();
        let zoomValue = slider.property("value");
